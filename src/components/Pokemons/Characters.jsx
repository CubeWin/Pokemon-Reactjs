...
const Character = () => {
	...
	const { getSearchCharacter, getCharacters, characters } =
	...
	const handleSubmit = e => {
		e.preventDefault();
		getSearchCharacter(pokemon.toLowerCase());
		setPokemon('');
	};

	return (
		<div className='col-span-1 lg:col-span-2 xl:col-span-3 pb-5'>
			<div className='bg-gray-100 rounded-2xl my-5 mx-2 flex justify-between'>
				<form onSubmit={handleSubmit} className='flex justify-center'>
					<label className='relative block h-10 w-96 my-auto mx-2 overflow-hidden'>
			...
			{loading ? (
				...
				<div className='flex flex-wrap justify-center lg:justify-between'>
					{characters.length > 0 ? (
						characters.map(pl => <Card key={pl.id} {...pl} />)
					) : (
						<div className='w-full h-full flex justify-center'>
							<h1 className='text-2xl mt-5'>Pokémon no encontrado</h1>
						</div>
					)}
				</div>
			)}
